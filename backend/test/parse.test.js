...
const good = JSON.stringify({
  ...
  notes: 'Bright and smoky, on the drier side.',
...
test('throws on empty ingredients', () => {
  const bad = JSON.stringify({ name: 'X', method: 'shaken', steps: 'x', ingredients: [] });
  assert.throws(() => parseRecipe(bad), /non-empty array/);
});

test('throws on ingredient with non-positive amount', () => {
  const bad = JSON.stringify({ name: 'X', method: 'shaken', steps: 'x', ingredients: [{ name: 'gin', amount: 0, unit: 'oz' }] });
...
test('tolerates missing optional fields (garnish/notes/balance_check)', () => {
  const minimal = JSON.stringify({
    name: 'Plain', method: 'stirred', steps: 'Stir.',
